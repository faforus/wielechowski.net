...
function Home() {
  ...
  useEffect(() => {
    ...
    if (indexWithoutAppropriateProportion !== undefined) {
      console.log(
        `Image at index ${indexWithoutAppropriateProportion} does not have an appropriate proportion.`
      );
    } else {
      console.log("All images have appropriate proportions.");
    }
  ...
  useEffect(() => {
    ...
    sortedImages.forEach((item, index) => {
      ...
      if (
        ...
        mappedImgs.push(
          <div className={classes.grid} key={`grid-${index + Math.random()}`}>
            {gridImgs}
          </div>
        );
        ...
        doesNotFitCount > 0 &&
        (index + 1) % 14 === doesNotFitCount % 14
      ) {
        // create a new div without the grid class and add remaining items with doesNotFit set to true
        mappedImgs.push(
          <div key={`grid-${index}`}>{gridImgs.splice(-doesNotFitCount)}</div>
        );
      }
    });
